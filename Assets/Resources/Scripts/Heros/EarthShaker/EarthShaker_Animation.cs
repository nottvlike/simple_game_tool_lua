...
public class EarthShaker_Animation : MonoBehaviour {
	...
	void Start () {
		...
		attackEvent.time = _anims ["mdldecompiler.qc_skeleton|attack1"].length * 2 / 3;
		attackEvent.functionName = "onAttackFinished";
		_anims ["mdldecompiler.qc_skeleton|attack1"].clip.AddEvent ( attackEvent );
	}
...
//		animation ["mdldecompiler.qc_skeleton|idle"].
	...
	public void goAttack()
	{
		state = 2;
		Debug.Log ("goAttack");
		_anims ["mdldecompiler.qc_skeleton|attack1"].speed = _anims ["mdldecompiler.qc_skeleton|attack1"].length * (1 + AttackSpeed / 100) / BasicBetween;
		_anims.Play ("mdldecompiler.qc_skeleton|attack1");
//		anim.Play ("Attack_Normal");
	...
	{
		if (skillid == 3) {
			_anims.Play("mdldecompiler.qc_skeleton|enchant_totem");		
		}
